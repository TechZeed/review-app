...
describe('apps/ui appconfig', () => {
  ...
  it('dev config has non-empty firebase fields', () => {
    ...
    expect(devJson.firebase.storageBucket).not.toBe('');
    expect(devJson.firebase.messagingSenderId).not.toBe('');
    expect(devJson.firebase.appId).not.toBe('');
  });

  it('prod config has emailLogin=false (safety: no internal email login in prod)', () => {
